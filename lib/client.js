 ...
 * - 极简高质感：区分局域网/公网出口 IP，显示显式代理状态，端口分组与悬浮释放。
 *
	...
	factory: (require) => {
		...
		const LOCALES = {
			zh: {
				...
				networkSection: '本机网络接口',
				localIp: '局域网 IP',
				publicIp: '公网出口 IP',
				proxyStatus: '代理状态',
				proxyConnected: '检测到显式代理配置',
				proxyNotDetected: '未检测到显式代理配置',
				publicIpNotChecked: '未检测（不会自动联网）',
				detectPublicIp: '检测出口 IP',
				checkingPublicIp: '检测中…',
				copyIp: '复制 IP',
				copied: '已复制',
				portsSection: '活跃监听端口',
			...
			en: {
				...
				localIp: 'LAN IP',
				publicIp: 'Public egress IP',
				proxyStatus: 'Proxy status',
				proxyConnected: 'Explicit proxy configured',
				proxyNotDetected: 'No explicit proxy configured',
				publicIpNotChecked: 'Not checked (no automatic network request)',
				detectPublicIp: 'Check egress IP',
				checkingPublicIp: 'Checking…',
				copyIp: 'Copy IP',
				copied: 'Copied',
		...
		function ensureStyle() {
			...
			style.textContent = [
				...
				// 卡片 Grid
				'.dsh-env-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,480px),1fr));gap:18px;margin-bottom:20px}',
				'.dsh-env-card{background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l2);border-radius:10px;',
				'padding:18px 20px;min-width:0;display:flex;flex-direction:column;box-shadow:0 1px 3px rgba(0,0,0,.03);transition:border-color .18s}',
				...
				// CLI 工具网格
				'.dsh-env-cli-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}',
				'.dsh-env-cli-item{display:flex;align-items:center;justify-content:space-between;background:var(--dsw-alias-bg-layer-3);',
				'border:1px solid var(--dsw-alias-border-l3);border-radius:8px;padding:7px 12px;min-width:0;gap:8px;font-size:12px;transition:border-color .15s}',
				'.dsh-env-cli-item:hover{border-color:var(--dsw-alias-border-l2)}',
				'.dsh-env-cli-name{font-weight:600;color:var(--dsw-alias-label-primary);display:flex;align-items:center;gap:6px;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}',
				'.dsh-env-cli-ver{color:var(--dsw-alias-label-secondary);font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}',
		...
		function HeroStatsRow(report, t) {
			...
			const localIp = net.localIp ?? net.primaryIp ?? '127.0.0.1'
		 ...
		 * 专门列**AI CLI / IDE / 代理**：codex / gemini / claude / opencode / agy /
		 * aider / cursor / continue / cody / tabby / goose / cline / lm-studio / ollama。
		 *
		 * 普通 CLI 只保留 node、git、docker 等基础工具，AI 工具统一在这里显示。
		 */
		function AiToolsCard(ai, t) {
			const list = Array.isArray(ai) ? ai.filter((c) => c.ok && c.version) : []
			return h('div', { className: 'dsh-env-card' },
				h('div', { className: 'dsh-env-card-head' },
					h('span', { className: 'dsh-env-card-title' },
						h('span', { style: { fontSize: '16px' } }, '🤖'),
					 ...
					 h('span', { className: 'dsh-env-badge-pill' }, `${list.length} ${t('aiInstalled')}`)
				),
				h('div', { className: 'dsh-env-cli-grid' },
					...list.map((c) => h('div', {
						key: c.name,
		...
		/**
		 * 编程语言版本卡片，只展示成功探测到版本的运行时。
		 */
		function LanguageToolsCard(languages) {
			const list = Array.isArray(languages) ? languages.filter((c) => c.ok && c.version) : []
			return h('div', { className: 'dsh-env-card' },
				h('div', { className: 'dsh-env-card-head' },
					h('span', { className: 'dsh-env-card-title' }, h('span', { style: { fontSize: '16px' } }, '🧩'), '语言运行时'),
					h('span', { className: 'dsh-env-badge-pill' }, `${list.length} 已安装`)
				),
				h('div', { className: 'dsh-env-cli-grid' },
					...list.map((c) => h('div', { key: c.name, className: 'dsh-env-cli-item', title: `${c.name} ${c.version}` },
						h('span', { className: 'dsh-env-cli-name' }, h('span', { className: 'dsh-env-status-dot', style: { background: 'var(--dsw-alias-state-success-primary,#10b981)', boxShadow: 'none' } }), c.name),
						h('span', { className: 'dsh-env-cli-ver' }, c.version)
					))
				)
			)
		}
		...
		function PluginsAndNetworkCard(plugins, net, t) {
			...
			const localIp = net?.localIp ?? net?.primaryIp ?? (ifaces.find((i) => !i.internal && i.family === 'IPv4')?.address ?? '127.0.0.1')
			const proxyConfigured = net?.proxy?.configured === true
			const [publicIp, setPublicIp] = React.useState(null)
			const [checkingPublicIp, setCheckingPublicIp] = React.useState(false)
			...
			function detectPublicIp() {
				...
				fetch('https://api.ipify.org?format=json', { signal: AbortSignal.timeout(5000) })
					.then((response) => response.ok ? response.json() : Promise.reject(new Error('public IP request failed')))
					.then((data) => setPublicIp(typeof data?.ip === 'string' ? data.ip : null))
					.catch(() => setPublicIp(null))
					.finally(() => setCheckingPublicIp(false))
			}

			function copyIp() {
				if (navigator.clipboard && localIp) {
			...
			return h('div', { className: 'dsh-env-card', style: { gridColumn: '1 / -1' } },
				...
				h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '24px' } },
					...
					// 右栏：网络主 IP 突出展示与物理网卡
					h('div', null,
						h('div', { style: { fontSize: '11px', fontWeight: 600, color: 'var(--dsw-alias-label-secondary)', marginBottom: '8px' } }, t('localIp')),
						h('div', { className: 'dsh-env-ip-hero' },
							h('span', { className: 'dsh-env-ip-val' }, localIp),
							h('button', {
								type: 'button',
						...
						h('div', { className: 'dsh-env-keyval-row', style: { margin: '8px 0' } },
							h('span', { className: 'dsh-env-keyval-k' }, t('publicIp')),
							h('span', { className: 'dsh-env-keyval-v' }, publicIp ?? t('publicIpNotChecked')),
							h('button', { type: 'button', className: 'dsh-env-copy-btn', onClick: detectPublicIp, disabled: checkingPublicIp }, checkingPublicIp ? t('checkingPublicIp') : t('detectPublicIp'))
						),
						h('div', { className: 'dsh-env-keyval-row' },
							h('span', { className: 'dsh-env-keyval-k' }, t('proxyStatus')),
							h('span', { className: 'dsh-env-badge-pill' }, proxyConfigured ? t('proxyConnected') : t('proxyNotDetected'))
						),
		...
		function EnvInspectorView(props) {
			...
			return h('div', { className: 'dsh-env-inspector__view-container' },
				...
				h('div', { className: 'dsh-env-grid' },
					...
					LanguageToolsCard(report.languages ?? []),
		...
		function EnvInspectorBadge(props) {
			...
			return h('div', { className: 'dsh-env-inspector', style: { position: 'relative' } },
				...
				open && report ? h('div', { className: 'dsh-env-inspector__panel' },
					...
					h('div', { className: 'dsh-env-inspector__row' },
						h('span', { className: 'dsh-env-inspector__key' }, 'LAN IP'),
						h('span', { className: 'dsh-env-inspector__val' }, report.network?.localIp ?? report.network?.primaryIp ?? '127.0.0.1')
					),
					h('div', { className: 'dsh-env-inspector__row' },
