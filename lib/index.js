...
function probeCliTools() {
	const tools = ['node', 'npm', 'pnpm', 'yarn', 'git', 'cs', 'docker', 'mise', 'brew']
...
/**
 * 探测**广义 AI CLI 工具链** —— 普通 CLI 之外、本机常见的 AI 助手 / IDE / 代理。
 *
 * 设计：
 *   - **AI CLI 单独归类**：Codex、Gemini、Claude、OpenCode 等统一在 AI 工具链展示。
 *   - **按 PATH 查询 + `--version` 探测**：纯本地，零联网。
 *   - **多个常见路径**：aider / cursor / continue 等通常装在 ~/.local/bin 或 /usr/local/bin，
 *     普通 PATH 在 node 进程的 process.env.PATH 里能拿到。
 *   - **fallback**：装过的工具 `command -v` 返 0 但 `--version` 返 1 的话，
 *     这里算 ok=false，UI 标记 "未装"。不假装装了。
 */
function probeAiTools() {
	const tools = ['codex', 'gemini', 'claude', 'opencode', 'agy', 'aider', 'cursor', 'continue', 'cody-cli', 'tabby', 'codestral', 'goose', 'cline', 'lm-studio', 'ollama']
	return tools.map((name) => {
		const { ok, version } = probe(name, ['--version'])
		return { name, ok, version }
	})
...
/**
 * 探测常用编程语言运行时，仅用于显示已经安装且能返回版本的语言。
 * @returns `{ name, ok, version }[]`
 */
function probeLanguageTools() {
	const tools = [{ name: 'python', command: 'python3' }, { name: 'go', command: 'go' }, { name: 'rust', command: 'rustc' }]
	return tools.map(({ name, command }) => {
