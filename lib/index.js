 ...
 *
 * 设计：
 *   - **不重复 probeCliTools**：CLI tools 列表里已有的 `codex / claude / agy` 在这里**省略**，
 *     避免 UI 上同一条信息出现两次。
...
function probeAiTools() {
	const tools = ['aider', 'cursor', 'continue', 'cody-cli', 'tabby', 'codestral', 'opencode', 'goose', 'cline', 'lm-studio', 'ollama']
	return tools.map((name) => {
		const { ok, version } = probe(name, ['--version'])
		return { name, ok, version }
	})
 ...
 * 过滤虚拟网络噪音（utun/awdl/llw/bridge 等），提炼物理主要网卡及本机局域网 IP。
 * @returns object
 */
function readNetworkSummary() {
	...
	return {
		primaryIp: primaryIp || (ifaces.find(i => !i.internal && i.family === 'IPv4')?.address ?? '127.0.0.1'),
		interfaces: ifaces
	}
}
