...
import { hostname, platform, arch, release, cpus, totalmem, freemem, networkInterfaces, uptime } from 'node:os'
...
function probeInstalledPlugins() {
	try {
		...
		const profilesRoot = process.env.DSH_PROFILES_DIR ?? '/Users/webkubor/.dsh/profiles'
...
export function probeListeningPorts() {
	try {
		const result = spawnSync('lsof', ['-iTCP', '-sTCP:LISTEN', '-n', '-P'], {
			encoding: 'utf8',
			timeout: 1500,
			stdio: ['ignore', 'pipe', 'pipe']
		})
