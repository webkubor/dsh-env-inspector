...
### 3. 🔐 AI 凭证与秘钥安全透明度（零明文泄漏）
- **环境变量存在性标签**：核对 DeepSeek、OpenAI、Anthropic、Google、Moonshot、智谱等 10 款主流模型 Key 是否已配；
- **Kyvault 密钥库收纳统计**：统计本地已加密收纳的 Secret 引用；
- **安全红线**：所有接口与组件**绝不输出、不读取、不传递任何密钥明文**。
